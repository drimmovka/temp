...
void ImagePNG::allocateMemmoryForRowPointers()
{
    row_pointers_ = (png_bytepp) malloc(sizeof(png_bytep) * height_);
    for (int y = 0; y < height_; y++) {
        row_pointers_[y] = (png_bytep) malloc(pixel_size_  * width_);
    }
}

void ImagePNG::freeMemmoryForRowPointers()
{
    for (int y = 0; y < height_; y++) {
...
void ImagePNG::clearRowPointers()
{
    for (int y = 0; y < height_; y++) {
