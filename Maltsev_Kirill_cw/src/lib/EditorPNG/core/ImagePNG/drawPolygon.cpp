...
void getIntersections(std::vector<std::pair<int, int>>& intersections, int y, std::vector<Coord>& vertices)
{
...
void ImagePNG::fillPolygon(std::vector<Coord>& vertices, Color& fill_color)
{
    int y_min = INT_MAX;
    int y_max = 0;
    for (int i = 0; i < vertices.size(); i++) {
...
void ImagePNG::drawPolygon(std::vector<Coord> vertices, int thickness, 
