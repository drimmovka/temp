...
Sentence* input_sentence(int* check_error) {
    Sentence* new_sent = (Sentence*)malloc(sizeof(Sentence));
    new_sent->str = (wchar_t*)calloc(STR_SIZE, sizeof(wchar_t));
    new_sent->size_str = 0;
    int check_input = 1;
    int count_bsn = 0;
    do {
        wchar_t new_symbol = getwchar();
        if ((new_sent->size_str % STR_SIZE) == STR_SIZE - 2) {
            new_sent->str = (wchar_t*)realloc(new_sent->str, (new_sent->size_str + STR_SIZE) * sizeof(wchar_t));
        } 
        if (new_symbol == L'\n') {
            count_bsn++;
        }
        ...
        if ((new_symbol == L' ' || new_symbol == L'\t' || new_symbol == L'\n') && new_sent->size_str == 0 && count_bsn < 2) {
            continue; 
        }
        if (new_symbol == L'.' && new_sent->size_str == 0) {
            error(VOID_STR); 
            check_input = 0;
            *check_error = 0;
        }
        if (count_bsn == 2 && new_sent->size_str != 0) {
            error(END_DOT);
            check_input = 0;
            *check_error = 0; 
        }
        ...
        if (check_input == 1) {
            new_sent->str[new_sent->size_str] = new_symbol;
            new_sent->size_str++;
            if (new_symbol == L'.') {
                check_input = 0;
            }
    ...
    if (new_sent->str != NULL) {
        new_sent->str[new_sent->size_str] = L'\0';
    }
    return new_sent;
}
